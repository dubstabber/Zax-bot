"""Core scratch fields: dump/menu/spawn state, the per-bot state
block, fire/aim + lead-shot working state, name/color tables,
snapshot tags and weapon dispatch."""

from .model import BOT_STATE_FIELDS, ScratchField, _bot_state_block


def extend_core(c):
    num_bot_names = c.num_bot_names
    name_slot_size = c.name_slot_size
    name_slot_ascii = c.name_slot_ascii
    weapon_speeds_max = c.weapon_speeds_max
    force_bot_ammo_max = c.force_bot_ammo_max
    force_bot_ammo_slot_size = c.force_bot_ammo_slot_size
    MAX_BOT_SLOTS = c.MAX_BOT_SLOTS
    fields = c.fields
    bot_state_end = c.bot_state_end
    bot_state_fields = c.bot_state_fields
    ...
    c.MAX_BOT_SLOTS = MAX_BOT_SLOTS
    c.fields = fields
    c.bot_state_end = bot_state_end
    c.bot_state_fields = bot_state_fields
